...
dotnet run -- test --printer network --host printer.local:5251  # over the network, via the ESP32 firmware (or a PC running ReceiptPrinter.NetworkSerialService)
...
- `--host` - network service host:port (e.g., `printer.local:5251` for the ESP32, or `printer-pc.local:5251` for a PC stand-in)
